...
SOURCES=terms.md order-form.md
...
build/%.docx: build/%.form.json build/%.title build/%.edition build/%.signatures styles.json | build $(DOCX)
	$(DOCX) --title "$(shell cat build/$*.title)" --edition "$(shell cat build/$*.edition)" --signatures build/$*.signatures --number outline --indent-margins --left-align-title --styles styles.json $< > $@
...
build/%.signatures: %.md | build $(COMMONMARK) $(JSON)
	$(COMMONMARK) parse < $< | $(JSON) frontMatter.signatures > $@
